...
class PurchaseOrder extends AuditedEntity {
    ...
    String termsAndConditions, notes;
...
class PurchaseOrderItem {
    ...
    UUID productId, unitId;
    String description, hsnSacCode;
    BigDecimal quantity,
            rate,
...
class GoodsReceipt extends AuditedEntity {
    ...
    UUID supplierId, purchaseOrderId, warehouseId;

    @Column(name = "receipt_date")
    LocalDate receiptDate;

    String status = "DRAFT";
    boolean inventoryPosted;
...
class GoodsReceiptItem {
    ...
    UUID productId, unitId;
    String description, batchNumber;
...
class PurchaseInvoice extends AuditedEntity {
    ...
    LocalDate invoiceDate, dueDate;
    UUID supplierId, purchaseOrderId, goodsReceiptId, warehouseId;
    String placeOfSupply, supplierGstin, status = "DRAFT", paymentStatus = "UNPAID";
    boolean reverseCharge, taxInclusive;
    BigDecimal subtotal = BigDecimal.ZERO,
            discountTotal = BigDecimal.ZERO,
            taxableAmount = BigDecimal.ZERO,
    ...
    @Column(columnDefinition = "text")
    String notes, termsAndConditions;
...
@Setter
@NoArgsConstructor
class PurchaseReturn extends AuditedEntity {
    @Column(name = "return_number")
    String returnNumber;

    UUID purchaseInvoiceId, supplierId;
...
class PurchaseReturnItem {
    ...
    UUID productId;
    BigDecimal quantity, rate, lineTotal;
    Integer lineOrder = 0;
}
...
@Setter
@NoArgsConstructor
class DebitNote extends AuditedEntity {
    @Column(name = "debit_note_number")
    String debitNoteNumber;

    UUID purchaseReturnId, purchaseInvoiceId, supplierId;
...
class PurchaseOrderService {
    ...
    PurchaseOrderService(DocumentNumberService n, OrganizationRepository o) {
        numbers = n;
        organizations = o;
    }

    PurchaseOrder create(OrderRequest request) {
    ...
    PurchaseOrder update(UUID id, OrderRequest request) {
...
class GoodsReceiptService {
    ...
    GoodsReceiptService(PurchaseOrderService o, DocumentNumberService n, OrganizationRepository r, InventoryService i) {
        orders = o;
        numbers = n;
        organizations = r;
        inventory = i;
    ...
    GoodsReceipt fromPurchaseOrder(UUID poId, GrnRequest r) {
...
class PurchaseInvoiceService {
    ...
    PurchaseInvoice fromGrn(UUID grnId, InvoiceRequest r) {
    ...
    PurchaseInvoice fromPo(UUID poId, InvoiceRequest r) {
    ...
    PurchaseInvoice confirm(UUID id) {
...
class PurchaseController {
    ...
    PurchaseController(PurchaseOrderService o, GoodsReceiptService g, PurchaseInvoiceService i) {
        orders = o;
        grns = g;
        invoices = i;
    }
    ...
    PurchaseOrder createOrder(@Valid @RequestBody OrderRequest r) {
    ...
    PurchaseOrder order(@PathVariable UUID id) {
        return orders.get(id);
    }

    @PutMapping("/orders/{id}")
    PurchaseOrder updateOrder(@PathVariable UUID id, @Valid @RequestBody OrderRequest r) {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    GoodsReceipt fromOrder(@PathVariable UUID poId, @Valid @RequestBody GrnRequest r) {
        return grns.fromPurchaseOrder(poId, r);
    }

    @PostMapping("/grn/{id}/confirm")
    GoodsReceipt confirmGrn(@PathVariable UUID id) {
        return grns.confirm(id);
    }

    @GetMapping("/grn")
    List<GoodsReceipt> grns() {
        return grns.list();
    }

    @PostMapping("/invoices/from-order/{poId}")
    PurchaseInvoice invoiceFromPo(@PathVariable UUID poId, @Valid @RequestBody InvoiceRequest r) {
        return invoices.fromPo(poId, r);
    }

    @PostMapping("/invoices/from-grn/{grnId}")
    PurchaseInvoice invoiceFromGrn(@PathVariable UUID grnId, @Valid @RequestBody InvoiceRequest r) {
        return invoices.fromGrn(grnId, r);
    }

    @PostMapping("/invoices/{id}/confirm")
    PurchaseInvoice confirmInvoice(@PathVariable UUID id) {
