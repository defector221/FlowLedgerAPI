...
public class LoyaltyPromotionGenerator {
    ...
    public LoyaltyPromotionGenerator(
            RetailLoyaltyService retailLoyaltyService, RetailPricingService retailPricingService) {
    ...
    public void generate(DemoSeedContext ctx, ProgressLogger progress) {
        ...
        if (blueprint.workflow().loyaltyEnabled()) {
            try {
                retailLoyaltyService.createTier(new TierRequest("SILVER", "Silver", new BigDecimal("500"), new BigDecimal("1")));
                retailLoyaltyService.createTier(new TierRequest("GOLD", "Gold", new BigDecimal("2000"), new BigDecimal("1.5")));
        ...
        progress.done("Loyalty & promotions (" + created.size() + " bundles)");
    }
}
