...
public final class AmountInWords {
    ...
    public static String inr(BigDecimal amount) {
        BigDecimal v=amount.abs().setScale(2, RoundingMode.HALF_UP); long rupees=v.longValue(); int paise=v.remainder(BigDecimal.ONE).movePointRight(2).intValue();
        String result=(amount.signum()<0?"Minus ":"")+words(rupees)+" Rupees"+(paise>0?" and "+words(paise)+" Paise":"")+" Only";
        return result.replaceAll("\\s+", " ").trim();
    }
    private static String words(long n) {
        if(n==0)return "Zero"; if(n>=10_000_000)return words(n/10_000_000)+" Crore "+words(n%10_000_000);
